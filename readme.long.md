...
- Want speed: use the cached data pipeline (`hextts/data/cached_dataset.py`) and precompute features.
...
If confusion reaches critical mass, check the `diagram/` and `docs/` folders before declaring the project haunted.
...
```
HexTTs/
├── scripts/train.py           ← The actual sorcery happens here
├── scripts/infer.py           ← "Please make sounds from my text"
├── scripts/main_flow.py       ← One CLI to rule your workflow and reduce typo-based suffering
│
├── hextts/models/             ← The neural network brain lives here now
├── hextts/data/               ← Data loading and caching internals
├── hextts/inference/          ← Text-to-audio pipeline internals
├── scripts/view_spectrogram.py← Visualize mel spectrograms (stare at pretty graphs)
│
├── configs/base.yaml          ← "How angry should my GPU get?"
├── requirements.txt           ← All the suffering, listed as pip packages
├── CHANGELOG.md               ← A record of your poor decision-making over time
│
├── scripts/prepare_data.py    ← "Let me fix the phonemes because the dataset was messy"
├── scripts/validate_dataset.py← Quality control (spoiler: data is weird)
├── scripts/precompute_features.py ← Computes mel spectrograms ahead of time so training doesn't die slowly
...
├── docs/                      ← Extra documentation you'll read later (spoiler: you won't)
...
```bash
venv\Scripts\python.exe scripts/run_continuation_test.py --epochs 3 --resume-checkpoint checkpoints/checkpoint_step_090000.pt
```
...
Good news: this is already wired in the current package pipeline, so you do **not** need to manually edit imports in `scripts/train.py` anymore.

Just run precompute once, then train normally. The loader path in `hextts/data/` will consume cached features automatically.
...
```bash
python scripts/main_flow.py infer --text "Hello world" --hifigan --output tts_output/hello_world.wav
```

Repeat the command with different `--text` values until your robot voice satisfies your inner audio critic.
...
**Fix:** Open `configs/base.yaml`, set `batch_size` to 4, then 2, then 1 if you must. Yes, 1. We don't judge. (We judge a little.)
...
### "ModuleNotFoundError: No module named 'hextts'"

Did you run the command from repo root? Did you activate your virtual environment? Did you install dependencies? Did `hextts/` survive your last refactor frenzy?
...
### "scripts/precompute_features.py ran but training is still slow"

Check that cache files actually exist under `data/ljspeech_prepared/cache/` and that your config points to the prepared dataset path. If not, you're training the hard way.
...
| -------------------------------- | -------------------------------------------------------------------------- | ----------------------------------------------------------- |
| `scripts/train.py`               | Training entrypoint that launches the 45M-parameter speaking crisis        | Always. Forever. It haunts your nightmares                  |
| `scripts/infer.py`               | "Please convert my text to robot sounds" (it tries its best, bless it)     | After training (or when you give up)                        |
| `scripts/main_flow.py`           | One-command workflow wrapper for less typing and fewer CLI faceplants      | When your command history looks like a legal document       |
| `hextts/models/`                 | The actual neural network brain (touch it and it breaks, don't ask how)    | Rarely, unless you enjoy controlled chaos                   |
| `hextts/data/`                   | Data pipeline internals (raw + cached loaders, collate, prep helpers)      | During training setup and speed tuning                      |
| `configs/base.yaml`              | "How much suffering do I want today?" (knobs to destroy your GPU)          | Before training (read it carefully)                         |
| `scripts/prepare_data.py`        | Converts Linda Johnson's words into phoneme soup (alphabet chaos)          | Once. Then pray it worked. Then run it again when it didn't |
| `scripts/validate_dataset.py`    | "Is my data broken?" (spoiler: yes, but in acceptable ways)                | When paranoid. Which is always.                             |
| `scripts/precompute_features.py` | Pre-bakes mel spectrograms so training doesn't die (PLEASE RUN THIS)       | Before training (seriously, do it)                          |
...
| `docs/`                          | Extended documentation (the README you never read, but should)             | When README isn't enough (it will be)                       |
...
1. Takes SAMPLE_TEXTS from the training pipeline in `scripts/train.py` (pre-written audition lines)
...
Running `scripts/infer.py --text "Hello" --output hello.wav`:
...
When you run `python scripts/train.py --config configs/base.yaml --device cuda`, expect a wall of text like this:
...
When you run `python scripts/infer.py --checkpoint checkpoints/best_model.pt --text "Hello world"`:
...
`scripts/infer.py` now supports an optional neural vocoder path.
...
**If you have an RTX 3050 Ti:** Set `batch_size: 4`, enable `use_amp: true`, run `scripts/precompute_features.py` first, and accept that your GPU is doing its absolute best. Respect it.
...
Total parameters depend on the exact model/config version. Check startup logs for the real number in your run.
...
_Last updated: April 15, 2026_ (v0.5.0 — docs synced with package refactor and current script paths)  
_GPU cooling status: CRITICAL_  
_Electricity bill status: DO NOT OPEN_  
_Cached pipeline status: Use it. Seriously._
