...
Usage
-----
  python audit_dataset.py --config vits_config.yaml

  # Dry-run: see report without writing filtered files
  python audit_dataset.py --config vits_config.yaml --dry-run

  # Adjust thresholds from the command line
  python audit_dataset.py --config vits_config.yaml --min-duration 0.4 --max-duration 12.0
...
Examples:
  ...
  python audit_dataset.py --config vits_config.yaml --min-duration 0.8 --max-duration 8.0

  # Allow more silence (useful for datasets with natural pauses)
  python audit_dataset.py --config vits_config.yaml --max-silence 0.75
        """,
    )
